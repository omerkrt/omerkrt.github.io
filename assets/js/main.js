...
const projectData = {
  flexview: {
    ...
    description: 'FlexView maps structural results and sizing information onto the FE model in standalone HTML reports. It visualizes SOL 101 and SOL 105 results and prepares S2D reports for designers after fatigue sizing contributions are incorporated. Used between 150+ analysis and 70+ design engineers on the TF-X program, it saves an estimated 10,000 engineering hours annually through visualization and S2D reporting.',
    features: [
  ...
  ijat: {
    ...
    description: 'i-JAT coordinates HyperMesh load extraction, Excel-based joint sizing, postprocessing, and CSV/XLSX reporting for composite and metallic aircraft joints. Its outputs are ready for FlexView. Used by 35 engineers, it saves an estimated 6-14 hours of hands-on work per engineer per iteration. The annual estimate of 4,000+ engineering hours counts only the 20 required iterations per engineer.',
    features: [
  ...
  sade: {
    ...
    description: 'SADE extracts von Mises stress results from Nastran models, classifies structural elements by connection type (CBUSH, T-junction, L-bend), applies appropriate safety factors, and calculates reserve factors with thickness optimization. It saves an estimated 1,000 engineering hours annually: 20 engineers, 20 required iterations each, and about 2.5 hours saved per iteration.',
    features: [
      'Intelligent element classification by connection type with appropriate safety factors',
      'Vectorized processing of element-case combinations',
  ...
  bade: {
    ...
    description: 'BADE extracts and analyzes buckling mode data from Nastran OP2 or PCH files. It identifies maximum displacement nodes, maps them to structural properties, detects coplanar surfaces and T-connections, and calculates optimized thicknesses. It saves an estimated 1,000 engineering hours annually: 20 engineers, 20 required iterations each, and about 2.5 hours saved per iteration.',
    features: [
  ...
  huth: {
    ...
    description: 'Calculates and updates CBUSH element stiffness values in Nastran BDF files using the empirical Huth formula for fastened joints, including stiffness updates after plate thicknesses change during sizing. Features composite material support with Classical Lamination Theory, multi-plate connection analysis, and Airbus-method dual-CBUSH generation.',
    features: [
      ...
      'Batch processing of CBUSH stiffness values across full-aircraft models',
  ...
  propmapper: {
    ...
    description: 'Transfers properties between different FE mesh configurations using proximity scoring, surface normal alignment, and ray casting for tie-breaking. It saves an estimated eight hours of hands-on work per mapping task. With 20 engineers completing about three tasks each per year, the estimated annual saving is 480 engineering hours.',
    features: [
  ...
  rbemapper: {
    ...
    description: 'Automatically generates RBE2 and RBE3 elements by mapping master nodes to slave nodes, with exclusive assignment, priority-based conflict resolution, and colinearity detection. In a representative RBE3 generation case, processing time fell from about six hours with my earlier TCL script to about ten seconds. This is a reduction in unattended processing time.',
    features: [
  ...
  thermalmapper: {
    ...
    description: 'Maps thermal loads from coarse/source meshes to fine/target meshes using k-nearest neighbor interpolation with inverse distance weighting. Enables coupled thermal-structural FEA workflows by transferring temperature distributions between non-conforming meshes. Used by about 10 engineers for about 20 iterations each per year, its benefit is shorter processing waits.',
    features: [
  ...
  cfhm: {
    ...
    description: 'A HyperMesh plugin for the TF-X Center Fuselage team, automating FEM model preparation, fastener modeling, quality checks, and analysis workflows. Features 6 tabbed tool categories with 75+ procedures built collaboratively by the analysis team. An estimated half-hour saved per week for 50 engineers over 48 working weeks amounts to 1,200 engineering hours annually.',
    features: [
